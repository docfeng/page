/*
      ...
      names() 获取数据库所以表名称;return array
      delete_db() 删除数据库 return true false
      close_db() 关闭数据库 return true;
      create_store(name,json)创建数据表 name 名称 json {key:"name",index:{name:true,val:false}}
          return:false 失败;create新建;open 打开
      select_store(name) 选择数据表 return true false;
      clear_store() 清空数据表
      delete_store() 删除数据表
      add(json,key)修改key值
      put(json,key?)添加修改key return true false      
      delete(key)删除key
      count()记录数量
      getAll()获取全部记录
      getkey(key) return json
      getindex(json) {name:66} 
      getindexkeys(json){name:66}
      getcursor
      update_cursor
